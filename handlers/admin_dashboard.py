...
from telegram.ext import (
    ...
    MessageHandler,
    Filters  # ⚠️ If using PTB v20+, replace with `filters`
)
...
def admin_dashboard(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        stats, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
def broadcast_message(update: Update, context: CallbackContext):
    if (
        update.effective_user.id in broadcast_mode
        and broadcast_mode[update.effective_user.id]
    ):
